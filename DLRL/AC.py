...
class ActorCritic:
    ...
    def candidate_ques(self,know, init_diff):
        ...
        if closest_difficulty.difficulty == None: closest_difficulty.difficulty = 0
        for item in all_ques:
            if item.difficulty == None: item.difficulty = 0
    ...
    def take_action(self, know, state, init_diff=None):
        ...
        candidates = know
        if len(candidates) == 1: return know[0]
        candidate_probs = probs.gather(dim=1, index=torch.Tensor(candidates).long().view(1, -1).to(device))
        ques = int(random.choices(candidates, weights=candidate_probs.cpu().view(-1).detach().numpy())[0])
        return ques

...
class LearnerGroup(MetaInfinityLearnerGroup):
    ...
    def __next__(self):
        ...
        # self.random_state = np.random.RandomState(10)
        # if not os.path.isdir(self.data_path) and not 'npz' in self.data_path:
        with open(self.data_path, 'r', encoding="utf-8") as f:
                self.datatxt = f.readlines()
        ...
        session = json.loads(self.datatxt[self.student_id])
        initial_log = copy.deepcopy(session)
        #取学生的回答情况
        second_elements = [row[1] for row in session]
        if set(second_elements) == {1}:
            last_zero_index = len(second_elements) - 1
        else:
            last_zero_index = len(second_elements) - 1 - second_elements[::-1].index(0)

        learning_targets = {initial_log[last_zero_index][0]}

...
class MyDataset(Dataset):
    ...
    def get_selected_data(self):
        if self.data_path != f'{get_proj_path()}/dataProcess/MOOCCube/student_log_kt_None':
            raise ValueError('This function can only be used under env KT training situation')

        env_selected_train_data_path = f'{get_proj_path()}/dataProcess/MOOCCube/student_log_kt_None_selected'
        selecte_count = 0
...
class EnvKTtrainer:
    def __init__(self, train_goal='env_KT'):
        ...
        if self.env_selected_data_train:
            self.env_KT_data_path = f'{get_proj_path()}/dataProcess/MOOCCube/student_log_kt_None_selected'
            print(self.env_KT_data_path)
        else:
            self.env_KT_data_path = f'{get_proj_path()}/dataProcess/MOOCCube/student_log_kt_None'
        self.agent_KT_data_path = f'{get_proj_path()}/dataProcess/MOOCCube/dataRec'
        ...
        if self.train_goal == 'env_KT':
            ...
            self.weight_path = f'{get_proj_path()}/EduSim/Envs/KES_MOOCCube/meta_data/env_weights/'
            os.makedirs(self.weight_path, exist_ok=True)
        else:
            self.data_path = self.agent_KT_data_path
            self.weight_path = f'{get_proj_path()}/EduSim/Envs/KES_MOOCCube/meta_data/agent_weights/'
            os.makedirs(self.weight_path, exist_ok=True)
        ...
        self.val_dataloader = DataLoader(dataset=self.train_dataset, batch_size=self.batch_size, shuffle=False)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_num):
            ...
            #     if i % 20 == 0:
            #         print(f"epcoh:{epoch + 1}  iteration:{i}   ")
    ...
    def test(self, name, dataloader):
        ...
        for i, batch_data in enumerate(dataloader):
            # if batch_data.shape[0] < 200: continue
    ...
    def compute_loss(self, output, batch_data):
        ...
        for episode in range(batch_data.shape[0]):
            ...
            target_id = torch.roll(target_id, -1, 0).unsqueeze(1).unsqueeze(0).to(device)  # [1, sequence_length, 1]

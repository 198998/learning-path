...
def build_graph(base_data_path, graph_type, KT_graph_path=None, model_type='GKT'):
    ...
    print(graph)
    np.save(f'{get_raw_data_path()}/dataProcess/MOOCCube/MyTrainsitionGraph.npy',graph)
...
if __name__ == '__main__':
    base_data_path = f'{get_raw_data_path()}/dataProcess/MOOCCube/student_log_kt_None'
    build_graph(base_data_path, 'Transition')
    num_skills = 442

    graph = np.load(f'{get_raw_data_path()}/dataProcess/MOOCCube/MyTrainsitionGraph.npy')
    ...
    with open(f"{get_proj_path()}/data/dataProcess/MOOCCube/nxgraph.pkl", "wb") as file:
